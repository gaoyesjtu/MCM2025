...
class EnhancedMedicalRiskFunction:
    ...
    def plot_risk_function(self, reach_time=11, test_time_range=(3, 35), save_path=None):
        ...
        # 主曲线
        ax.plot(test_times, risks, 'b-', linewidth=3, label='增强风险函数')

        # 达标时间
        ax.axvline(x=reach_time, color='red', linestyle='--', linewidth=2,
                   label=f'达标时间 = {reach_time} 周')

        # 早期/晚期区域填充
        early_times = test_times[test_times < reach_time]
        late_times = test_times[test_times > reach_time]

        if early_times.size > 0:
            early_risks = [self.calculate_individual_risk(t, reach_time) for t in early_times]
            ax.fill_between(early_times, early_risks, alpha=0.3, color='green', label='早期检测 (二次函数)')
        ...
        ax.set_ylabel('风险值')
        ax.set_title(f'增强医学风险函数 (达标时间 = {reach_time} 周)')
